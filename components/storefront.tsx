...
export function Storefront({ products, initialCustomer }: { products: Product[]; initialCustomer: Customer | null }) {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <header className="relative z-20 border-b border-border bg-background/95 px-5 py-5 backdrop-blur md:px-10">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-6">
          ...
          <nav className={`${menuOpen ? 'flex' : 'hidden'} absolute left-0 top-full w-full flex-col gap-5 border-b border-border bg-background px-6 py-6 text-xs uppercase tracking-[0.18em] md:static md:flex md:w-auto md:flex-row md:border-0 md:bg-transparent md:p-0`}>
            <a href="#shop" onClick={scrollToSection('shop')} className="hover:text-olive">Novidades</a><a href="#shop" onClick={scrollToSection('shop')} className="hover:text-olive">Coleções</a><a href="#story" onClick={scrollToSection('story')} className="hover:text-olive">Alma</a>
          </nav>
          <a href="#top" onClick={scrollToSection('top')} className="font-serif text-3xl italic tracking-[-0.06em]">alma<span className="text-terracotta">.</span></a>
      ...
      <section className="bg-sage px-5 py-16 md:px-10 md:py-24"><div className="mx-auto max-w-[1400px]"><div className="mb-10 flex items-end justify-between"><div><p className="text-[10px] uppercase tracking-[0.2em] text-olive">Escolha seu caminho</p><h2 className="mt-3 font-serif text-5xl tracking-[-0.06em]">Feito para agora.</h2></div><a href="#shop" onClick={scrollToSection('shop')} className="hidden text-xs uppercase tracking-[0.15em] underline underline-offset-4 md:block">Ver tudo</a></div><div className="grid gap-3 md:grid-cols-3">{categories.map((category) => <a href="#shop" onClick={scrollToSection('shop')} key={category.label} className="group relative h-[360px] overflow-hidden bg-olive md:h-[460px]"><img src={category.image} alt={category.label} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-ink/20" /><span className="absolute bottom-6 left-6 font-serif text-3xl italic text-primary-foreground">{category.label}</span><ArrowRight className="absolute bottom-7 right-6 text-primary-foreground" size={21} /></a>)}</div></div></section>
      ...
      <section id="story" className="grid bg-ink text-primary-foreground md:grid-cols-2"><div className="flex flex-col justify-center p-8 md:p-20"><p className="text-[10px] uppercase tracking-[0.2em] text-sage">Manifesto Alma</p><h2 className="mt-5 max-w-lg font-serif text-5xl leading-[0.95] tracking-[-0.05em] md:text-7xl">Menos pressa. <em>Mais presença.</em></h2><p className="mt-7 max-w-md text-sm leading-7 text-primary-foreground/70">A Alma nasceu da vontade de desacelerar o vestir. Criamos pequenas coleções, produzidas em ritmo humano, para você construir um guarda-roupa que tenha a sua cara e o seu tempo.</p><a href="#newsletter" onClick={scrollToSection('newsletter')} className="mt-8 flex w-fit items-center gap-3 text-xs uppercase tracking-[0.16em] text-sage">Conheça nossa história <ArrowRight size={15} /></a></div><img src="https://images.unsplash.com/photo-1525507119028-ed4c629a60a3?auto=format&fit=crop&w=1200&q=85" alt="Araras com roupas selecionadas no ateliê Alma" className="h-[500px] w-full object-cover md:h-[680px]" /></section>

      <section className="bg-terracotta px-5 py-14 text-primary-foreground md:px-10 md:py-20"><div className="mx-auto flex max-w-[1400px] flex-col justify-between gap-8 md:flex-row md:items-end"><div><p className="text-[10px] uppercase tracking-[0.2em] text-primary-foreground/70">Só até domingo</p><h2 className="mt-3 max-w-xl font-serif text-5xl leading-none tracking-[-0.06em] md:text-7xl">Seu primeiro passo começa aqui.</h2></div><div className="md:text-right"><p className="mb-5 text-sm text-primary-foreground/80">15% off na primeira compra</p><a href="#shop" onClick={scrollToSection('shop')} className="inline-flex items-center gap-3 bg-ink px-6 py-4 text-[10px] font-semibold uppercase tracking-[0.18em] text-primary-foreground">Usar meu desconto <ArrowRight size={15} /></a></div></div></section>

      <footer id="newsletter" className="bg-background px-5 py-14 md:px-10 md:py-20"><div className="mx-auto grid max-w-[1400px] gap-12 md:grid-cols-[1.4fr_1fr_1fr]"><div><a href="#top" onClick={scrollToSection('top')} className="font-serif text-4xl italic tracking-[-0.06em]">alma<span className="text-terracotta">.</span></a><p className="mt-5 max-w-xs text-sm leading-6 text-muted-foreground">Roupas para estar inteira em tudo o que você faz.</p><form className="mt-8 flex max-w-sm border-b border-foreground/50 pb-3" onSubmit={(e) => e.preventDefault()}><input placeholder="Seu melhor e-mail" aria-label="Seu melhor e-mail" type="email" className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" /><button aria-label="Assinar newsletter"><ArrowRight size={18} /></button></form></div><div><p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Navegue</p><div className="mt-5 flex flex-col gap-3 text-sm"><a href="#shop" onClick={scrollToSection('shop')}>Novidades</a><a href="#shop" onClick={scrollToSection('shop')}>Coleções</a><a href="#story" onClick={scrollToSection('story')}>Nossa história</a><a href="#newsletter" onClick={scrollToSection('newsletter')}>Contato</a></div></div><div><p className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Siga a Alma</p><div className="mt-5 flex gap-5 text-sm"><a href="#newsletter" onClick={scrollToSection('newsletter')}>Instagram</a><a href="#newsletter" onClick={scrollToSection('newsletter')}>Pinterest</a></div><p className="mt-10 text-xs text-muted-foreground">© 2026 Alma Studio. Feito no Brasil.</p></div></div></footer>
